...
void gpsdataSet()
{
    Data data = {70.500444, 44.500355, "verigeldi"}; // Gönderilecek veri Paketi
    ...
    while (GPSSerial.available() > 0)
        if (gps.encode(GPSSerial.read()))
        {
            if (true) //gps.location.isValid()
            {
                ...
                //double lat = gps.location.lat();     // lat=enlem verileri gönderebilmek için değişkene atadım
                //Serial.print(gps.location.lat(), 6); // enlem
                //Serial.print(F(","));
                //double lng = gps.location.lng();     // lng=boylam verileri gönderebilmek için değişkene atadım
